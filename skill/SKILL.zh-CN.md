...
## 表是心脏:永远维护 state.json

`~/.claude/skills/gh/state.json` 是本 skill 唯一的记忆。**任何能力的每次调用,结束前都要把刚学到的写回去**:取全了一个 issue → upsert 它的条目(state/评论数/assignees/updatedAt/lastSeenAt);跑了盘点 → 全量刷新自己的条目;发现某条 closed/merged → 记下。读很便宜,表过期很贵——写回一步不许省。字段分工:

- **自己条目的名单不手工维护**——每次用 `gh search --author=@me` 现场重新发现(新开的自动进、关掉的自动出)。表只负责记"上次见到它长什么样"(用来算 diff 和球权翻转)。
- `"watch": true` 是唯一手工的东西:**别人的**、用户明说要盯的条目。绝不自动盯;按能力三自清洁。
...
输出用两张 markdown 表格:先「需要你行动」(列:项目|条目(带链接)|状态|卡点|下一步),后「等对方」(列:项目|条目|状态|球在谁|等了多久)。会随 PR 自动关闭的配套 issue 可折叠成一行。末尾取数覆盖。
